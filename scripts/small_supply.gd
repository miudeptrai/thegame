...
const HEAL_RANGE_TILE = preload("uid://d3dsohta1m4dt");
...
var active_heal_range_tile: Array = [];
...
func load_range(pos: Vector2i) -> void:
	#Setting up heal range tile and deploying it
	#Preventing the troop pos
	if (pos == Vector2i(0, 0)): return;
	
	var curr: Area2D;
	var avail_arr: Array = map.heal_range_avail;
	if (not avail_arr.is_empty()):
		curr = avail_arr.pop_back();
	else:
		curr = HEAL_RANGE_TILE.instantiate();
		map.add_child(curr);
	curr.owner_of_action = owner_of_action;
	curr.skill = self;
	active_heal_range_tile.append(curr);
...
func deselect(is_action: bool) -> void:
	...
	var avail_arr: Array = map.heal_range_avail;
	for i in range(active_heal_range_tile.size()):
		avail_arr.append(active_heal_range_tile.pop_back());
